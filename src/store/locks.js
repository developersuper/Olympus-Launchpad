...
import {FACTORY_ADR_LP, FACTORY_ADR_TOKENS} from "@/js/constants";
...
export default {
    ...
    actions: {
        async loadLockList(context){
            let locks = await getAllLocks(context.rootState.web3, FACTORY_ADR_LP);
        ...
        async loadHolderLocks(context, holder){
            let locks = await getLockHoldings(context.rootState.web3, holder, FACTORY_ADR_LP);
        ...
        async loadLock(context, lock){
            try {
                let info = await loadLPLockInfo(context.rootState.web3, context.rootState.bnbPrice, lock);
