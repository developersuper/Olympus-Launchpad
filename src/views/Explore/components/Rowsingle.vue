<template>
  <router-link @click="$router.push(`/launchcard/${id}`)" to="" class="mb-5 px-10 py-6 bg-gray-800 lg:py-3 lg:px-3 lg:mb-0 flex lg:flex-row flex-col justify-between items-center lg:bg-transparent hover:bg-opacity-50 hover:bg-gray-700 rounded-lg lg:mx-0 mx-4">
    <div class="tokenNameCell mb-2 lg:pb-0 pb-4 flex items-center">
      ...
      <span v-if="isPublic" class="px-2 py-1 rounded-md bg-gray-500 font-bold text-gray-900 text-xs ml-2 xl:hidden">PUB</span>
      <span v-if="!isPublic" class="px-2 py-1 rounded-md bg-gray-500 font-bold text-gray-900 text-xs ml-2 xl:hidden">PRI</span>
    ...
    <div class="text-sm justify-between w-full flex launchRatioCell">
      <span class="lg:hidden mb-1 inline-block md:text-base font-semibold text-gray-200 pr-2">Ratio</span>
      <span class="text-launchpad_primary font-semibold md:text-base lg:text-xs lg:text-gray-100 lg:font-normal" v-html="'1 BNB = ' + bogintific(ratio)"></span>
    </div>
    <div class="text-sm justify-between w-full flex launchParticipantsCell">
      ...
      <span class="text-launchpad_primary font-semibold md:text-base lg:text-xs lg:text-gray-100 lg:font-normal">{{ participants }}</span>
    </div>
    <div class="text-sm justify-between w-full flex launchTypeCell">
      ...
      <span class="text-launchpad_primary font-semibold md:text-base lg:text-xs lg:text-gray-100 lg:font-normal">{{ launchType }}</span>
    </div>
    <div class="text-sm lg:text-xs justify-between w-full flex launchProgressCell">
      ...
      <div class="countdown lg:pr-4 lg:w-full caption">
        ...
        <div class="hidden lg:block h-3 bg-gray-700 rounded-full w-full">
          <div class=" w-full rounded-full bg-gradient-to-r to-launchpad_primary-dark h-full from-launchpad_primary" v-bind:style="`width:${progress}%`"></div>
        </div>
      </div>
    </div>
    <div class="text-sm lg:text-xs justify-between w-full flex launchEndsCell">
      ...
      <div class="countdown lg:pr-4 lg:w-full caption">
        <TimeLine1 :starttime="startDate" :endtime="endDate" />
      </div>
    </div>
...
export default {
  props: ["isLive", "id", "name", "icon", "ratio", "participants", "launchType", "isOwned", "isPublic", "partnerType", "progress", "startDate", "endDate"],
  name: "Rowsingle",
  components: {
    TimeLine1,
    "vue3-slider": slider,
